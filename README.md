...
Миграции встроены в `mintrud-admin` и автоматически применяются до открытия
HTTP-порта. При обычном перезапуске уже применённые миграции не выполняются
повторно.

Технические команды используют тот же `MINTRUD_ADMIN_DB`, что и сервер:

```bash
go -C backend run ./cmd/mintrud-admin db status
go -C backend run ./cmd/mintrud-admin db migrate
go -C backend run ./cmd/mintrud-admin db verify
go -C backend run ./cmd/mintrud-admin db backup
```
...
`MINTRUD_ADMIN_ENV=prod` путь `MINTRUD_ADMIN_DB` обязан быть абсолютным.
