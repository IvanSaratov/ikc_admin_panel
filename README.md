...
При наличии pending migrations backend сначала выполняет `quick_check`, создаёт и
проверяет backup, затем применяет SQL и повторяет проверки. При ошибке HTTP не
запускается. SQLite-файл без выпущенного `application_id` не изменяется и
отклоняется как неизвестная база. Не восстанавливайте backup поверх открытой базы.
